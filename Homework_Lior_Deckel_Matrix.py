...
test_Api_BaseURI = "https://pokeapi.co/api/v2/"
Pokemon_Cnt = 20

# function for retrieving a url's response.
def retrieve_url(my_url):
    print(f"Perform request.Get() on url : {my_url}")
    response = requests.get(my_url) # get the data from the URL
...
# ---------
response = retrieve_url(test_Api_BaseURI+'type')
...
assert poke_fire_type_url != "", f"Failed to find a URL for the 'fire' type Pokemons."

# this pokemon SHOULD be present in the Fire-Type
poke_present = 'charmander'

# this pokemon SHOULD NOT be present in the Fire-Type
poke_absent = 'bulbasaur'

response_fireType = retrieve_url(poke_fire_type_url)
...
print(f"response_fireType.text = {response_fireType.text}") # just for debugging...
resp_json_fireType = json.loads(response_fireType.content.decode('utf-8')) # used for easier handling/parsing
...
for poke_name in dict_Fire_HeavyWeight.keys():
    ...
    for json_poke_name in resp_json_fireType['pokemon']:
        ...
        if json_poke_name['pokemon']['name'] == poke_name:
            ...
            response_heavyweight = retrieve_url(heavy_pokemonURL)
            ...
            # verify the weights in the Json are the as expected from the dictionary
    ...
    print(f"Verified all 'heavyweight' fire-type Pokemons.")
